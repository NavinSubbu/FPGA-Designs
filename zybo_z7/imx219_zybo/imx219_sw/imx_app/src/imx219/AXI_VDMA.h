...
#include "xaxivdma.h"
#include "xil_printf.h"   // use xil_printf for bare-metal prints
...
public:
    ...
    void resetRead()
    {
//      XAxiVdma_ChannelStop(&drv_inst_.ReadChannel);   //unused - left for reference
//      while (XAxiVdma_ChannelIsRunning(&drv_inst_.ReadChannel)) ;
    ...
    void resetWrite()
    {
//      XAxiVdma_ChannelStop(&drv_inst_.WriteChannel);  //unused - left for reference
//      while (XAxiVdma_ChannelIsRunning(&drv_inst_.WriteChannel)) ;
    ...
    void configureRead(uint16_t h_res, uint16_t v_res)
    {
        ...
        // Clear errors in SR and enable read-channel interrupts
    ...
    // Interrupt handlers print simple status messages (bare-metal friendly)
    ...
    {
        xil_printf("VDMA: write error (mask=0x%08x)\r\n", mask);
    }
    ~AXI_VDMA() = default;
private:
    XAxiVdma drv_inst_;
